...
def power_law(x,k,slope,intercept):
	return intercept + np.exp(np.log(k) + slope*np.log(x))
...
def plot_degree_distribution(graph):
	...
	k, slope, intercept = tuple(popt)
	plt.scatter(deg,cnt)
	plt.plot(deg,power_law(deg,k,slope,intercept),\
