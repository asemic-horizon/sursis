...
cmap = "PuOr_r"
with nc() as conn: db.calculate_energy(conn)
node_mode = "Nodes"
...
if op_mode == node_mode:
	...
	if node and add_button: 
		with nc() as conn: found = db.write_node(conn,node)
		ui.if_confirm(found,"Name already exists")
	if node and del_button:
		with nc() as conn: found = db.del_node(conn,node)
		ui.if_confirm(found)
	...
	with nc() as conn: nodes = db.list_nodes(conn)
	node_1 = ui.known_field_input("Source","jazz")
	node_2 = ui.known_field_input("Target","espionage")
	add_button, del_button = ui.add_del()
	if node_1 and node_2 and add_button: 
		with nc() as conn: db.write_edge(conn, node_1,node_2)
		ui.confirm()
	if node_1 and node_2 and del_button:
		found = True
		while found:
			with nc() as conn: found = db.del_edge(conn,node_1, node_2)
	...
	if node_1 and node_2 and nonn_button:
		with nc()as conn:
			db.write_node(conn,node_2); ui.confirm()
			db.write_edge(conn,node_1,node_2); ui.confirm()
	...
	if node_1 in nodes and node_2 in nodes:
		...
		if merge_button:
			with nc() as conn: db.merge_nodes(conn,node_1,node_2, new_node)
			ui.confirm()
	...
	if ego:
		...
		with nc() as conn: fields = db.list_nodes(conn)
		u = fields.index("jazz")
